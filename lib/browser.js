...
function detectBrowser() {
  // An explicit path wins over everything. The packaged desktop build sets this to the
  // Chromium it ships, so a machine with no browser at all still works out of the box.
  const forced = process.env.JOBFINDER_BROWSER_PATH;
  if (forced && fs.existsSync(forced)) return { name: 'Bundled Chromium', path: forced };
