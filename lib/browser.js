...
// Windows was missing entirely, so every Windows install silently fell through to the
// bundled Chromium — a fresh profile with no history, which anti-bot systems score worse
// than a real installed browser. %LOCALAPPDATA% first: Chrome installs there per-user by
// default, and that copy is the one with the logins.
const WIN_BROWSERS = () => {
...
const CANDIDATE_BROWSERS = process.platform === 'win32' ? WIN_BROWSERS()
  : process.platform === 'linux' ? LINUX_BROWSERS
    : MAC_BROWSERS;

function detectBrowser() {
  // An explicit path wins over everything — for the rare case of pointing at a specific
  // build deliberately.
  const forced = process.env.JOBFINDER_BROWSER_PATH;
  if (forced && fs.existsSync(forced)) return { name: 'Chosen browser', path: forced };

  // A real installed browser next. This is not a preference for its own sake: that
  // browser has history, cookies and a profile that looks lived-in, and anti-bot systems
  // score exactly that. A freshly unpacked Chromium looks like what it is.
  for (const b of CANDIDATE_BROWSERS) {
    if (b.path && fs.existsSync(b.path)) return b;
  }

  // Only then the browser we ship, so a machine with nothing installed still works.
  // The packaged app sets FALLBACK rather than PATH for this reason — forcing it would
  // throw away the better option on every machine that has Chrome.
  const fallback = process.env.JOBFINDER_BROWSER_FALLBACK;
  if (fallback && fs.existsSync(fallback)) return { name: 'Bundled Chromium', path: fallback };
...
function sessionDir(profileId, _connector) {
  const profileRoot = dataPath('sessions', profileId);
...
function scanDir(profileId) {
  const dir = dataPath('sessions', profileId, 'scan');
...
/**
 * Seed the scan directory from the on-disk cookie store, BEFORE its Chrome starts.
 *
 * Order is the whole point. This used to run after the scan browser had launched, which
 * meant copying a cookie file underneath a Chrome that already had the empty one open —
 * Chrome keeps cookies in memory and writes them back, so the copy was silently undone
 * every time.
 *
 * It never showed up in testing because a Chrome was always already running during
 * development, which takes the CDP path above instead. It only bit on a cold start with
 * nothing open — which is exactly what happens on the machine of someone who has just
 * installed the app. The symptom was a scan that quietly returned only the logged-out
 * results: 30 LinkedIn jobs instead of 130, and no error anywhere.
 *
 * Chrome encrypts the cookie store with a key from the OS keychain, so "Local State"
 * has to travel with it — the key is referenced from there, and without it the copied
 * cookies decrypt to nothing.
 */
function seedScanCookiesFromDisk(profileId) {
  ...
  if (copied) console.log(`[browser] Seeded the background scan profile with your saved logins (${copied} file(s))`);
  return copied;
}
...
async function launchContext(profileId, key, { headless, stealth, offscreen }) {
  ...
  // Seed the scan profile from disk BEFORE its browser starts. Once Chrome owns the
  // directory it owns the cookie file too, and anything copied in is overwritten.
  if (offscreen && !runningPort) seedScanCookiesFromDisk(profileId);
...
export function hasSession(profileId, connector) {
  const p = dataPath('sessions', profileId, connector);
