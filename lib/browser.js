...
async function launchRealBrowser(browserPath, userDataDir, port, headless, stealth = true, { offscreen = false } = {}) {
  ...
  if (offscreen) {
    // OFF-SCREEN MODE: the window exists (so anti-bot systems see a normal Chrome
    // process with a real visible-to-the-OS viewport — keeps the Cloudflare-pass
    // rate of a visible window) but it's positioned thousands of pixels off-screen
    // so the user never sees it. This is what `scan` uses for background scraping.
    // Real screen size kept (1440x900) so fingerprint stays normal.
    args.push(
...
async function applyScripts(context, profileId, { isFirefox = false, stealth = true } = {}) {
  if (!stealth) {
...
//
// `offscreen`: spawn the Chrome window positioned off-screen (-32000,-32000) so the
// user never sees it but the OS still treats it as a real visible viewport (keeps
// anti-bot pass rate). Used by scan-all for the few connectors that need a browser.
// Login + apply windows use offscreen=false so the user can actually interact.
...
async function launchContext(profileId, key, { headless, stealth, offscreen }) {
  ...
  // Tear down any sibling for this profile — only one Chrome process per
  // user-data-dir is allowed. (Covers all combinations of S/N × V/O.)
  ...
  await applyScripts(entry.context, profileId, { isFirefox: entry.isFirefox, stealth });
