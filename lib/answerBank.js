...
const CANON = [
  ...
  // e[\s-]?mail, not e-?mail: normalisation has already turned every punctuation mark
  // into a space, so "E-mail ID" arrives as "e mail id" and the hyphen form never
  // matched. That silently gave "E-mail ID" its own key instead of the shared one.
  [/e[\s-]?mail/, 'email'],
...
/**
 * Collapse a field label to the one key its answer is stored under.
 *
 * `value`, when given, guards the personal fields. The rules below match the LABEL
 * alone, which is right for "E-mail address" and catastrophic for "Do you consent to
 * email updates?" — both contain "email", so both used to canonicalise to `email`, and
 * the second one wrote "Yes" over the address. Four fields on a real profile were
 * corrupted exactly that way.
 *
 * So when a value is being STORED and the label points at a personal field, the value
 * has to look like that field. If it does not, the answer keeps its own question as its
 * key and stays an ordinary answer — which is what it always was.
 *
 * Lookups pass no value and are unaffected: matching a form field to the email you have
 * on file must still work.
 */
export function canonicalKey(key, type, value) {
  const k = String(key || '').toLowerCase().replace(/[^a-z0-9 ]/g, ' ').replace(/\s+/g, ' ').trim();
  for (const [re, name] of CANON) {
    if (!re.test(k)) continue;
    if (value !== undefined && isIdentityKey(name) && !validateIdentity(name, value).ok) return k;
    return name;
  }
  if (type === 'email' && (value === undefined || validateIdentity('email', value).ok)) return 'email';
  if (type === 'tel' && (value === undefined || validateIdentity('phone', value).ok)) return 'phone';
  return k;
}
...
export async function recordAnswer(profileId, { field_key, label, value, type, source }) {
  ...
  // Re-derive the key from the LABEL, with the value in hand.
  //
  // The in-page observer has already collapsed the label to a key, and by then the
  // damage is done: "Do you consent to email updates?" arrives as `email`, and there is
  // no way to tell it apart from a real address field. The label is still here though,
  // so the decision is made again on the server where the value can be checked —
  // "Yes" is not an email address, so this stays the question it always was.
  field_key = canonicalKey(label || field_key, type, value) || field_key;
...
// ── Questions auto-apply could not answer ────────────────────────────────────
//
// When a form asks something the bank has no answer for, the alternative to parking
// it is guessing — and a guess here is not a cosmetic error. "How many years of
// Kubernetes do you have" answered by an LLM that has never met you is a false
// statement on a real job application, sent under your name.
//
// So the question is written to the bank with an EMPTY value and status
// 'needs_input'. Retrieval only ever reads 'approved' rows, so a parked question can
// never be used as an answer, by autofill or by the LLM, until you fill it in.
//
// Keyed by field_key like every other answer, so the same question asked by a second
// employer resolves to the same row: answer it once, and every job waiting on it
// unblocks together.
export async function parkQuestion(profileId, { field_key, label, type, options, jobId, replace = false }) {
  const key = canonicalKey(field_key, type);
  // A question is never a personal detail. Parking one under `email` would leave an
  // empty identity field that the next captured "Yes" could fill.
  if (isIdentityKey(key) && label && canonicalKey(label, type, 'x') !== key) {
    return { parked: false, reason: 'not a personal field' };
  }
  if (!key) return { parked: false, reason: 'no usable key' };

  const existing = await get(
    'SELECT field_key, value, status FROM answers WHERE profile_id = ? AND field_key = ?',
    [profileId, key]
  );
  // Never overwrite an answer you already gave, and never re-park one you rejected.
  //
  // `replace` is the one exception: the stored answer exists but the form will not
  // accept it (a value that is not among a dropdown's choices). Without this the
  // question is refused as already-known and silently never reaches you, while every
  // run keeps reporting it as the thing blocking the application.
  if (existing && existing.status === 'rejected') {
    return { parked: false, reason: 'you rejected this one' };
  }
  if (existing && existing.value && !replace) {
    return { parked: false, reason: 'already known' };
  }

  const opts = options && options.length ? JSON.stringify(options.slice(0, 60)) : null;
  if (existing) {
    await run(
      // Clear the value when re-parking. A row that is 'needs_input' must never keep a
      // stale value: retrieval ignores it, so it cannot be used, but it still LOOKS
      // answered in the bank — which is exactly how a question ends up permanently
      // stuck. needs_input means empty, always.
      "UPDATE answers SET label = COALESCE(NULLIF(?, ''), label), options = COALESCE(?, options), value = '', last_seen = ?, status = 'needs_input' WHERE profile_id = ? AND field_key = ?",
      [label || '', opts, Date.now(), profileId, key]
    );
  } else {
    await run(
      'INSERT INTO answers (profile_id, field_key, label, value, type, last_seen, hit_count, status, source, options, asked_by, asked_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',
      [profileId, key, label || key, '', type || 'text', Date.now(), 1, 'needs_input', 'auto-apply', opts, jobId || null, Date.now()]
    );
  }
  return { parked: true, field_key: key };
}

/** Questions waiting on you, newest first. */
export async function listNeedsInput(profileId) {
  const rows = await all(
    "SELECT * FROM answers WHERE profile_id = ? AND status = 'needs_input' ORDER BY asked_at DESC, last_seen DESC",
    [profileId]
  );
  return rows.map((r) => ({
    ...r,
...
export async function countNeedsInput(profileId) {
  const r = await get(
    "SELECT COUNT(*) AS n FROM answers WHERE profile_id = ? AND status = 'needs_input'",
    [profileId]
  );
  return r?.n || 0;
}
...
export async function upsertAnswer(profileId, fieldKey, value, label) {
  ...
  const changed = !existing || existing.value !== value;
  // An empty value is not an answer. Saving one must not mark the row approved, or a
  // question auto-apply parked would come back as "answered" with nothing in it — and
  // the next run would submit a form with a blank required field.
  const status = String(value || '').trim() ? 'approved' : 'needs_input';
  if (existing) {
    await run(
      "UPDATE answers SET value = ?, label = COALESCE(NULLIF(?, ''), label), last_seen = ?, status = ?, reviewed_at = ? WHERE profile_id = ? AND field_key = ?",
      [value, label || '', Date.now(), status, Date.now(), profileId, fieldKey]
    );
  } else {
    await run(
      'INSERT INTO answers (profile_id, field_key, label, value, type, last_seen, hit_count, status, source) VALUES (?,?,?,?,?,?,?,?,?)',
      [profileId, fieldKey, label || '', value, 'text', Date.now(), 1, status, 'you']
    );
  }
  if (changed) {
    const profile = await get('SELECT id, filters FROM profiles WHERE id = ?', [profileId]);
