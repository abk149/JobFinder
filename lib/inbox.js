...
export async function classifyEmail(profile, emailText) {
  const system = [
    ...
    '{"stage":"screening","summary":"one sentence on what they are asking for","action":"what the candidate should do next","date":"any date/time mentioned, verbatim, or empty string","confidence":"high|medium|low"}',
  ].join('\n');

  const raw = await chat(profile, [
    { role: 'system', content: system },
    { role: 'user', content: String(emailText || '').slice(0, 6000) },
  ], { num_predict: 900, temperature: 0.1, json: true, useSynthModel: true });
