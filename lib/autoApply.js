...
export async function resolveAnswer(profile, target, exactMap) {
  // Computed answers come first, and outrank the bank.
  //
...
export async function autoApplyRun(profile, { armed = false, limit = 10, perSite } = {}) {
  ...
  const summary = {
    ...
    applied: 0, dryRun: 0, needsInput: 0, skipped: 0, errors: 0, abandoned: 0,
    parked: [], results: [],
  };
  ...
  const FAILED = new Set(['error', 'needs_input']);

  async function record(job, r) {
    const blockedOn = (r.missing || []).map((q) => q.field_key).filter(Boolean);
  ...
  async function runPool(connector, jobs) {
    ...
    const workers = Array.from({ length: Math.min(width, jobs.length) }, async () => {
      ...
      // against the batch.
      while (usable < limit && next < jobs.length) {
        const job = jobs[next++];
        looked++;
