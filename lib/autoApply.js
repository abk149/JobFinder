...
//
// Boards differ in how fast they rot, so the cutoffs differ. Naukri listings turn over
// in days; LinkedIn postings stay live noticeably longer.
const MAX_AGE_DAYS = { naukri: 10, linkedin: 21 };
  ...
  return all(
    `SELECT * FROM jobs
      ...
      ORDER BY (? - COALESCE(discovered_at, 0)) / 86400000 ASC,
               ...
               discovered_at DESC
      LIMIT ?`,
    [profileId, ...conns, cutoff, Date.now(), limit]
  );
}
...
export async function autoApplyRun(profile, { armed = false, limit = 10, perSite } = {}) {
  const pid = profile.id;
  const pools = await eligibleByConnector(pid, limit);
  const summary = {
    armed,
    considered: Object.values(pools).reduce((n, r) => n + r.length, 0),
    perConnector: Object.fromEntries(Object.entries(pools).map(([c, r]) => [c, r.length])),
    applied: 0, dryRun: 0, needsInput: 0, skipped: 0, errors: 0, parked: [], results: [],
  };
  ...
  async function runPool(connector, jobs) {
    ...
    const width = armed ? 1 : Math.min(perSite || 3, 4);
    linfo(pid, `  ▸ ${connector}: ${jobs.length} job(s), ${width} at a time`);
    let next = 0;
    const workers = Array.from({ length: Math.min(width, jobs.length) }, async () => {
      while (next < jobs.length) {
        ...
        await record(job, r);
        // Pacing applies only when something was actually sent.
