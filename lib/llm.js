...
//   - separate embed() that calls /api/embeddings with nomic-embed-text for semantic retrieval.
...
export async function chat(profile, messages, { num_predict = 800, temperature = 0.3, keep_alive = KEEP_ALIVE, json = false, useSynthModel = false, timeoutMs } = {}) {
  const cfg = profileLLM(profile);
  const { url } = cfg;
  const model = useSynthModel ? await resolveSynthModel(profile) : cfg.model;
  let res;
  try {
    const body = {
      model,
