...
import { dataDir as jfDataDir } from './paths.js';
...
function getSqlite() {
  ...
  //   answers.kind — 'identity' for the handful of facts about you that never change
  //                   (name, contact, links, where you live), 'answer' for replies to
  //                   questions a form asked. They were one undifferentiated pile, and
  //                   that is how "Yes" ended up stored as an email address.
  for (const col of ['options TEXT', 'asked_by TEXT', 'asked_at INTEGER', "kind TEXT DEFAULT 'answer'"]) {
...
function getPg() {
  ...
  pgReady = (async () => {
    ...
    ALTER TABLE answers ADD COLUMN IF NOT EXISTS kind TEXT DEFAULT 'answer';
